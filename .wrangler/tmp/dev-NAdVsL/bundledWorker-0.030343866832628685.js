...
// .wrangler/tmp/bundle-XUNZqt/checked-fetch.js
...
var require_checked_fetch2 = __commonJS2({
  ".wrangler/tmp/bundle-eU6qyG/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
...
app.get("/api/sessions", async (c) => {
  ...
  try {
    ...
    $2("section.movie, .prog2__movie").each((i, movieEl) => {
      ...
      const todayWrapper = $2(movieEl).find(".prog-day__wrapper").first();
      const sessionContainer = todayWrapper.length > 0 ? todayWrapper : movieEl;
      $2(sessionContainer).find(".prog2__cont, .prog2__movie-session").each((j, sessionEl) => {
        ...
        const key = perfId || `${time}-${hall}-${title}`;
...
//# sourceMappingURL=bundledWorker-0.030343866832628685.js.map
