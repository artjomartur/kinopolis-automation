...
// .wrangler/tmp/bundle-JQGmzc/checked-fetch.js
...
var require_checked_fetch2 = __commonJS2({
  ".wrangler/tmp/bundle-A6KBoL/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
                        ...
                        <div style="background-color: #0a0a0d; padding: 40px 20px; font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif;">
                            <div style="max-width: 600px; margin: 0 auto; background: #141419; border: 1px solid rgba(255,255,255,0.08); border-radius: 32px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.5);">
                                <div style="padding: 40px; text-align: center; background: linear-gradient(180deg, rgba(255,255,255,0.03) 0%, transparent 100%);">
                                    ...
                                    <h1 style="color: #ffffff; font-size: 28px; font-weight: 800; margin-bottom: 16px; letter-spacing: -0.02em;">Passwort zur\xFCcksetzen</h1>
                                    <p style="color: #94a3b8; font-size: 16px; line-height: 1.6; margin-bottom: 32px;">
                                        Hallo ${user.first_name},<br>
                                        du hast eine Anfrage zum Zur\xFCcksetzen deines Passworts gestellt. Klicke auf den Button unten, um ein neues Passwort festzulegen.
                                    </p>
                                    <a href="${resetLink}" style="display: inline-block; background: linear-gradient(135deg, #e50914 0%, #ff3d47 100%); color: #ffffff; text-decoration: none; padding: 18px 36px; border-radius: 16px; font-weight: 800; font-size: 16px; box-shadow: 0 10px 25px rgba(229, 9, 20, 0.4);">
                                        Passwort jetzt \xE4ndern
                                    </a>
                                    <p style="color: #475569; font-size: 13px; margin-top: 32px;">
                                        Der Link ist aus Sicherheitsgr\xFCnden nur <strong>15 Minuten</strong> g\xFCltig.<br>
                                ...
                                <div style="padding: 20px; text-align: center; background: rgba(0,0,0,0.2); border-top: 1px solid rgba(255,255,255,0.05);">
                                    <p style="color: #475569; font-size: 11px; margin: 0; text-transform: uppercase; letter-spacing: 0.1em;">
                                        \xA9 2026 Kinopolis Automation Dashboard
                                    </p>
                                </div>
                            </div>
                        </div>
...
app.post("/api/auth/reset-password", async (c) => {
  try {
    ...
    let payload;
    try {
      payload = await verify2(token, JWT_SECRET);
    } catch (verifyErr) {
      console.error("JWT Verify Error:", verifyErr.message);
      return c.json({ error: "Der Link ist ung\xFCltig oder abgelaufen (JWT Error)" }, 401);
    }
    if (!payload.userId) {
...
app.post("/api/auth/change-password", async (c) => {
  ...
  try {
    const payload = await verify2(authHeader.split(" ")[1], JWT_SECRET);
    ...
    if (resendKey) {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Kinopolis Automation <hi@artjombecker.com>",
          to: payload.email,
          subject: `Schicht-Report: ${(/* @__PURE__ */ new Date()).toLocaleDateString("de-DE")}`,
          html: `
                        <div style="background-color: #0a0a0d; padding: 40px 20px; font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif;">
                            <div style="max-width: 600px; margin: 0 auto; background: #141419; border: 1px solid rgba(255,255,255,0.08); border-radius: 32px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.5);">
                                <div style="padding: 40px; text-align: center; background: linear-gradient(180deg, rgba(255,255,255,0.03) 0%, transparent 100%);">
                                    ...
                                    <div style="border-top: 1px solid rgba(255,255,255,0.05); padding-top: 24px; text-align: left;">
                                        ...
                                        <div style="display: flex; gap: 12px; margin-bottom: 8px;">
                                            <div style="background: rgba(229, 9, 20, 0.1); color: #e50914; padding: 6px 16px; border-radius: 50px; font-size: 13px; font-weight: 700;">${auslaesse} Ausl\xE4sse</div>
                                    ...
                                    <p style="margin-top: 40px; color: #475569; font-size: 12px;">
                                        Vielen Dank f\xFCr deinen Einsatz heute!<br>
                                        \xA9 2026 Kinopolis Automation
                                    </p>
                                </div>
                            </div>
...
app.get("/api/sessions", async (c) => {
  ...
  try {
    ...
    $2("section.movie, .prog2__movie").each((i, movieEl) => {
      ...
      $2(movieEl).find(".prog-nav__item[data-index]").each((navIdx, navEl) => {
...
var STATIC_MESSAGES = [
  {
    id: 1,
    title: "Mario Men\xFC & Merch Verkauf",
    content: "Hallo zusammen,\n\nmit dem Start des neuen Mario Films gehen wir mit mehreren Men\xFCs und Merch-Artikeln in den Verkauf.\n\nWICHTIG: Der 'Yoshi-Eimer' darf von Mitarbeitenden nicht gekauft werden. (Kein Mitarbeiterinnengeschenk, kein Einkaufspreis und auch nicht als Vollpreis).\n\nGrund daf\xFCr ist die schon jetzt sehr hohe Nachfrage von G\xE4sten gepaart mit der Tatsache, dass wir nur sehr wenige zugesendet bekommen haben.\n\nAlles ist, wie immer, buchbar \xFCber etwaige Barcodes auf den Produkten oder \xFCber das Touchscreen unter 'Packages' oder 'Merch'.",
...
app.get("/api/upcoming", async (c) => {
  try {
    ...
    const html3 = await response.text();
    const sliderMatch = html3.match(/<(?:section|div)[^>]*id="(?:coming-soon-slider|highlights)"[\s\S]*?<\/(?:section|div)>/);
    let blocks = [];
    if (sliderMatch) {
    ...
    if (blocks.length === 0) {
      const items = html3.match(/<(?:div|article)[^>]*class="[^"]*movie[^"]*"[^>]*>[\s\S]*?<\/(?:div|article)>/g);
...
app.post("/api/scan-plan", async (c) => {
  try {
    ...
    const prompt = `Du bist ein spezialisierter Assistent f\xFCr Kinobetriebe. 
        Analysiere dieses Foto eines gedruckten Auslassplans/Dienstplans.
        Extrahiere die Tabelle mit den Auslasszeiten (Credits).
        WICHTIG: Gib NUR ein raues JSON-Array zur\xFCck im Format: 
    ...
    if (!jsonMatch) {
      return c.json({ error: "KI konnte keine g\xFCltige Tabelle finden.", raw: result }, 500);
    }
              ...
              if (nextS && s.title !== nextS.title) {
                ...
                if (loc === "kp") {
                  ...
                  if (!existing) {
                    await env.DB.prepare("INSERT INTO notification_state (alert_hash) VALUES (?)").bind(alertHash).run();
                    await sendPushToAll(env, {
                      title: "\u{1F5BC}\uFE0F Plakatwechsel: " + hall.name,
                      body: `Film l\xE4uft seit 20 Min. Bitte Plakat f\xFCr "${nextS.title}" (${nextS.time} Uhr) einh\xE4ngen!`,
                      image: nextS.poster,
                      tag: "poster-alert"
                    }, loc);
                  }
    ...
    for (const row of scannedResults.results) {
      ...
      if (diff >= -5 && diff <= 5) {
        ...
        if (!existing) {
          await env.DB.prepare("INSERT INTO notification_state (alert_hash) VALUES (?)").bind(alertHash).run();
          await sendPushToAll(env, {
            title: "\u{1F5BC}\uFE0F Plakatwechsel: " + row.hall,
...
//# sourceMappingURL=bundledWorker-0.5614893587554023.js.map
