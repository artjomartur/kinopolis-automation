...
// .wrangler/tmp/bundle-ZksSs4/checked-fetch.js
...
var require_checked_fetch2 = __commonJS2({
  ".wrangler/tmp/bundle-BQA8yd/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
  ...
  try {
    ...
    if (location === "kp") {
      sessions = sessions.filter((s) => {
        return !(s.hall.includes("Helia") || s.hall.includes("Pali") || s.hall.includes("Rex") || s.hall.includes("Classic") || s.hall.includes("Broadway") || s.hall.includes("Bambi") || s.hall.includes("Festival"));
      });
    }
    const halls = {};
    sessions.forEach((s) => {
      ...
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: `Dieser Foto zeigt einen gedruckten Kinopolis 'Auslassplan'. Extrahiere die Tabelle und gib ausschlie\xDFlich ein valides JSON-Array zur\xFCck. Die Tabelle hat 5 Spalten: 1. Saal (z.B. Saal1), 2. Startzeit (HH:MM:SS), 3. Ende Credits (HH:MM:SS), 4. Ende Film (HH:MM:SS), 5. Filmtitel. Ignoriere Kopfzeilen. Das JSON soll folgende Struktur haben: [{ "hall": "...", "movie": "...", "start_time": "...", "credits_time": "...", "end_time": "..." }]. Antworte NUR mit dem JSON-String.` },
...
var worker_default = {
  ...
  async scheduled(event, env, ctx) {
    ...
    if (!env.DB) return;
    const baseUrl = "https://kinopolis-automation.artjombecker.com";
        ...
        if (diff > 0 && diff <= 15 && s.sold > 50) {
          ...
          if (!existing) {
            ...
            await sendPushToAll(env, {
              title: "\u{1F6C2} ZE N\xF6tig: " + hall.name,
        ...
        if (diff < 0 && diff >= -40 && diff <= -30) {
          ...
          if (!existing) {
            ...
            await sendPushToAll(env, {
              ...
              body: `${s.title} l\xE4uft seit 30 Min. Das Plakat kann jetzt gewechselt werden.`,
              tag: "poster-alert",
              data: { url: "/" }
            });
          }
...
//# sourceMappingURL=bundledWorker-0.4711914808727491.js.map
