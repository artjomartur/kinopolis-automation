...
// .wrangler/tmp/bundle-acjryV/checked-fetch.js
...
var require_checked_fetch2 = __commonJS2({
  ".wrangler/tmp/bundle-qxAer2/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
...
var require_boolbase = __commonJS2({
  "node_modules/boolbase/index.js"(exports, module) {
    var import_checked_fetch1272 = __toESM2(require_checked_fetch2());
    module.exports = {
...
var import_checked_fetch126 = __toESM2(require_checked_fetch2());
var import_checked_fetch124 = __toESM2(require_checked_fetch2());
var import_checked_fetch121 = __toESM2(require_checked_fetch2());
...
var sign2 = Jwt.sign;
var import_checked_fetch120 = __toESM2(require_checked_fetch2(), 1);
var static_exports = {};
...
var import_checked_fetch59 = __toESM2(require_checked_fetch2(), 1);
var esm_exports2 = {};
...
var import_checked_fetch119 = __toESM2(require_checked_fetch2(), 1);
...
var import_checked_fetch60 = __toESM2(require_checked_fetch2(), 1);
...
var import_checked_fetch61 = __toESM2(require_checked_fetch2(), 1);
var _a2;
...
var import_checked_fetch73 = __toESM2(require_checked_fetch2());
var import_checked_fetch71 = __toESM2(require_checked_fetch2());
var SelectorType;
...
var import_checked_fetch72 = __toESM2(require_checked_fetch2());
...
function getParse(parser) {
  return /* @__PURE__ */ __name2(/* @__PURE__ */ __name(function parse7(content, options, isDocument2, context) {
...
__export(forms_exports, {
  serialize: /* @__PURE__ */ __name(() => serialize2, "serialize"),
...
var import_checked_fetch118 = __toESM2(require_checked_fetch2(), 1);
var import_checked_fetch116 = __toESM2(require_checked_fetch2(), 1);
var import_checked_fetch113 = __toESM2(require_checked_fetch2(), 1);
...
var import_checked_fetch109 = __toESM2(require_checked_fetch2(), 1);
...
var import_checked_fetch110 = __toESM2(require_checked_fetch2(), 1);
...
var import_checked_fetch111 = __toESM2(require_checked_fetch2(), 1);
...
var import_checked_fetch112 = __toESM2(require_checked_fetch2(), 1);
var MIME_TYPES = {
  TEXT_HTML: "text/html",
...
var import_checked_fetch115 = __toESM2(require_checked_fetch2(), 1);
var import_checked_fetch114 = __toESM2(require_checked_fetch2(), 1);
var getCodePoint2 = (
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
...
var import_checked_fetch117 = __toESM2(require_checked_fetch2(), 1);
...
__name(fetchKinopolis, "fetchKinopolis");
__name2(fetchKinopolis, "fetchKinopolis");
...
var JWT_SECRET = "kinopolis-secret-2026";
...
__name(getEmailStyles, "getEmailStyles");
__name2(getEmailStyles, "getEmailStyles");
app.post("/api/auth/register", async (c) => {
  try {
    ...
    if (!email || !first_name || !last_name || !location || !password) {
      return c.json({ error: "Alle Pflichtfelder ausf\xFCllen" }, 400);
    }
    if (!c.env.DB) return c.json({ error: "Datenbank nicht verf\xFCgbar" }, 500);
                                ...
                                <div style="padding: 20px;">
                                    <div class="container">
                                        ...
                                        <div class="content">
                                            ...
                                            <p class="body-text">
                                                Dein Account f\xFCr das <strong>Kinopolis Automation Dashboard</strong> ist jetzt aktiv. Wir freuen uns, dich am Standort <strong>${location}</strong> im Team zu haben.
                                            </p>
                                            
                                            <div class="feature-card">
                                                <div class="feature-title">Deine neuen Tools</div>
                                                <div class="feature-item">\u{1F680} Echtzeit-Updates & Schichtpl\xE4ne</div>
                                                <div class="feature-item">\u{1F4F1} Digitaler Funk & Messenger</div>
                                                <div class="feature-item">\u{1F4D6} Handb\xFCcher & Checklisten</div>
                                            </div>
                                         ...
                                         <div class="footer">
                                             <p class="footer-text">
                                                 ...
                                                 \xA9 2026 Kinopolis Automation<br><br>
...
app.post("/api/auth/login", async (c) => {
  try {
    ...
    if (!user) return c.json({ error: "Ung\xFCltige Anmeldedaten" }, 401);
...
app.post("/api/auth/forgot-password", async (c) => {
  ...
  const user = await c.env.DB.prepare("SELECT id, first_name FROM users WHERE email = ?").bind(email.toLowerCase()).first();
  if (!user) {
    return c.json({ success: true, message: "Falls die E-Mail existiert, wurde ein Link gesendet." });
  }
  const resetToken = await sign2({
    userId: user.id,
...
app.post("/api/auth/reset-password", async (c) => {
  try {
    ...
    let payload;
    try {
      payload = await verify2(token, JWT_SECRET, "HS256");
    } catch (verifyErr) {
      const msg = verifyErr.message?.toLowerCase() || "";
      if (msg.includes("expired")) {
        return c.json({ error: "Der Link ist abgelaufen (\xE4lter als 60 Min.). Bitte fordere einen neuen an." }, 401);
      }
      return c.json({ error: "Der Link ist ung\xFCltig. Bitte fordere einen neuen an." }, 401);
    }
    if (!payload || !payload.userId) {
      return c.json({ error: "Ung\xFCltiger Token-Inhalt" }, 401);
    }
    const hash = await hashPassword(newPassword);
    const result = await c.env.DB.prepare("UPDATE users SET password_hash = ? WHERE id = ?").bind(hash, payload.userId).run();
...
app.get("/api/auth/me", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Ung\xFCltiger Token", message: e.message }, 401);
  }
});
app.post("/api/auth/change-password", async (c) => {
  ...
  try {
    const payload = await verify2(authHeader.split(" ")[1], JWT_SECRET, "HS256");
    const { newPassword } = await c.req.json();
    if (!newPassword || newPassword.length < 6) {
      return c.json({ error: "Passwort muss mindestens 6 Zeichen lang sein" }, 400);
    }
    const hash = await hashPassword(newPassword);
    await c.env.DB.prepare("UPDATE users SET password_hash = ? WHERE id = ?").bind(hash, payload.id).run();
    return c.json({ success: true });
  } catch (e) {
    return c.json({ error: "Fehler beim \xC4ndern des Passworts" }, 500);
  }
});
app.post("/api/email/subscribe", async (c) => {
  try {
    ...
    if (!email) return c.json({ error: "E-Mail erforderlich" }, 400);
    if (!c.env.DB) return c.json({ error: "DB nicht verf\xFCgbar" }, 500);
    await c.env.DB.prepare(
      "INSERT OR IGNORE INTO email_subscriptions (email, location) VALUES (?, ?)"
...
app.get("/api/email/unsubscribe", async (c) => {
  try {
    ...
    await c.env.DB.prepare("DELETE FROM email_subscriptions WHERE email = ?").bind(email.toLowerCase()).run();
    await c.env.DB.prepare("DELETE FROM users WHERE email = ?").bind(email.toLowerCase()).run().catch(() => {
    });
    return c.html(`
            <div style="font-family: sans-serif; text-align: center; padding: 50px;">
                <h1>Erfolgreich abgemeldet</h1>
                <p>Du wirst keine weiteren E-Mails mehr erhalten.</p>
                <a href="https://kinopolis.artjombecker.com" style="color: #e50914;">Zur\xFCck zum Dashboard</a>
            </div>
        `);
  } catch (e) {
  ...
  try {
    ...
    const mailRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Kinopolis Automation <hi@artjombecker.com>",
        to: payload.email,
        subject: `Kinopolis Schicht-Report: ${(/* @__PURE__ */ new Date()).toLocaleDateString("de-DE")}`,
        html: `
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
                    ...
                    <div style="padding:30px;">
                        ...
                        <div style="background:rgba(255,255,255,0.02);border-radius:18px;padding:24px;border:1px solid ${s.borderColor};">
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                                <span style="color:${s.textColor};font-size:14px;font-weight:500;">\u{1F4FD}\uFE0F Ausl\xE4sse</span>
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                                <span style="color:${s.textColor};font-size:14px;font-weight:500;">\u{1F9F9} Reinigungen</span>
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;">
                                <span style="color:${s.textColor};font-size:14px;font-weight:500;">\u{1F5BC}\uFE0F Poster-Checks</span>
                        ...
                        <div style="text-align:center;margin-top:40px;padding-top:30px;border-top:1px solid ${s.borderColor};">
                            <p style="color:#48484a; font-size:11px;line-height:1.6;">
                                ...
                                Viel Erfolg f\xFCr deine n\xE4chste Schicht!<br><br>
...
app.patch("/api/admin/users/:id/role", async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare("UPDATE users SET role = ? WHERE id = ? AND location = ?").bind(role, id, payload.location).run();
    return c.json({ success: true });
  } catch (e) {
...
app.delete("/api/admin/users/:id", async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare("DELETE FROM users WHERE id = ? AND location = ?").bind(id, payload.location).run();
    return c.json({ success: true });
  } catch (e) {
    return c.json({ error: "Fehler beim L\xF6schen des Nutzers" }, 500);
  }
});
app.get("/api/movie-details", async (c) => {
  ...
  try {
    ...
    const $2 = load(html3);
    const title = $2("h1.hl, h1, .movie__title, .movie-detail__title, .prog2__movie-title").first().text().trim();
    let synopsis = "";
    const synopsisSelectors = [
      ".filmdetail__content .text",
      "#filmdetail .text",
      ".movie__synopsis",
    ...
    for (const sel of synopsisSelectors) {
      const elements = $2(sel);
      elements.each((i, el) => {
        const t = $2(el).text().trim();
        if (t && t.length > 50 && !t.includes("Bitte w\xE4hlen Sie ein Kino aus")) {
          synopsis = t;
          return false;
        }
      });
    ...
    if (!synopsis || synopsis.length < 50) {
      const ogDesc = $2('meta[property="og:description"], meta[name="description"]').first().attr("content");
    ...
    if (!synopsis || synopsis.length < 50) {
      let longestP = "";
      $2("main p, article p, .content p, section p").each((i, el) => {
        const t = $2(el).text().trim();
    ...
    const castEl = $2(".movie-cast");
    let cast = "";
    if (castEl.length) {
      cast = castEl.text().replace(/\s+/g, " ").trim();
    }
    const durationMatch = html3.match(/(\d+)\s*Minuten/i) || html3.match(/(\d+)\s*Min\.?/i);
    const duration = durationMatch ? durationMatch[1] : null;
    const specs = $2(".movie__specs, .prog2__movie-info, .movie-detail__specs").text();
    const fskMatch = specs.match(/ab\s*(\d+)\s*Jahre/i) || specs.match(/FSK\s*(\d+)/i) || html3.match(/FSK[\s-]*(\d+)/i);
    const fsk = fskMatch ? `FSK ${fskMatch[1]}` : "FSK ?";
    const genreEl = $2('.movie__specs-el:contains("Genre"), .prog2__movie-info-item:contains("Genre"), [class*="genre"], .filmdetail__specs-item').first();
    const genre = genreEl.text().replace(/Genre:?/i, "").trim() || "Film";
    let trailerUrl = null;
    const videoId = $2("[data-video-id]").first().attr("data-video-id");
    if (videoId) {
      trailerUrl = `https://www.youtube.com/embed/${videoId}`;
    } else {
      const ytLink = $2('a[href*="youtube.com/watch"], a[href*="youtu.be"]').first().attr("href");
      if (ytLink) {
    ...
    return c.json({
      title,
      synopsis: synopsis || "Keine Beschreibung verf\xFCgbar.",
      cast,
      duration,
      fsk,
      genre,
...
app.get("/api/sessions", async (c) => {
  ...
  if (c.env.DB) {
    try {
      ...
      console.error("Archive table init failed:", e);
    }
  }
  try {
    ...
    const isTomorrow = dateStr === new Date(Date.now() + 864e5).toISOString().split("T")[0];
      ...
      if (matches) {
        ...
        if (idsAttr) {
          ...
          cleanIds.split(",").forEach((id) => {
            ...
            if (trimmed && trimmed.length > 5) allowedIds.add(trimmed);
          });
        }
      }
    });
    ...
    $2("section.movie, .prog2__movie").each((i, movieEl) => {
      ...
      let fsk = "FSK ?";
      const fskImg = $2(movieEl).find('img[src*="FSK"]').first().attr("alt");
      ...
      const detailLink = $2(movieEl).find('.hl-link, .prog2__movie-title, a[href*="/film/"]').first().attr("href");
      ...
      $2(movieEl).find(".prog2__cont, .prog2__movie-session").each((j, sessionEl) => {
        ...
        if (!hallTextContent) hallTextContent = $2(sessionEl).find(".prog2__hall-num").text().replace(/i$/, "").trim();
        ...
        const seatingAttr = $2(sessionEl).find("[data-seating]").attr("data-seating");
        ...
        const isBookable = !$2(sessionEl).hasClass("performance_expired") && !occupancyText.includes("nicht mehr buchbar") && !occupancyText.includes("ausverkauft");
        const sessionObj = {
          title,
          ...
          archived.results.forEach((a) => {
            ...
            if (!sessionMap.has(key)) {
              sessionMap.set(key, {
                ...
                capacity: a.capacity,
                isBookable: false,
                // Probably past or removed
                poster: null,
                // Poster might be missing in archive, but better than nothing
                duration: 120,
                // Fallback
                fsk: "FSK ?"
              });
...
var STATIC_MESSAGES = [
  { id: 0, title: "Willkommen", content: "Willkommen im Kinopolis Automation Dashboard. Nutze das \xDCbergabebuch f\xFCr wichtige Infos.", author: "System", created_at: (/* @__PURE__ */ new Date()).toISOString() }
];
...
app.post("/api/announcements", async (c) => {
  ...
  try {
    const payload = await verify2(authHeader.split(" ")[1], JWT_SECRET, "HS256");
    if (payload.role !== "BL" && payload.role !== "admin") {
      return c.json({ error: "Nur BL/Admin d\xFCrfen Nachrichten pinnen" }, 403);
    }
                                    ...
                                    <div style="padding: 20px;">
                                        <div class="container">
                                            ...
                                            <div class="content">
                                                ...
                                                <div style="text-align: center;">
                                                    <a href="https://kinopolis.artjombecker.com" class="btn">Dashboard \xF6ffnen</a>
                                                </div>
                                            </div>
                                            <div class="footer">
                                                 <p class="footer-text">
                                                     Du erh\xE4ltst diese E-Mail als Mitarbeiter von Kinopolis.<br>
                                                     \xA9 2026 Kinopolis Automation<br><br>
...
app.post("/api/mhd", async (c) => {
  try {
    ...
    if (c.env.DB) {
      await c.env.DB.prepare("INSERT INTO mhd_records (item_id, item_name, type, location, mhd_date, author) VALUES (?, ?, ?, ?, ?, ?)").bind(item_id || null, item_name, type, location, mhd_date, author || "System").run();
    }
    return c.json({ success: true });
  } catch (e) {
    return c.json({ error: e.message }, 500);
...
app.get("/api/task-completions", async (c) => {
  ...
  try {
    ...
    try {
      await c.env.DB.prepare("ALTER TABLE task_completions ADD COLUMN type TEXT").run();
    } catch (e) {
    }
    try {
      await c.env.DB.prepare("ALTER TABLE task_completions ADD COLUMN author TEXT").run();
    } catch (e) {
    }
...
app.post("/api/inventory/counts", async (c) => {
  try {
    const { location, date, type, data: data2, author } = await c.req.json();
    if (!location || !date || !type || !data2) return c.json({ error: "Missing data" }, 400);
    if (c.env.DB) {
      await c.env.DB.prepare(
        "INSERT INTO inventory_counts (location, date, type, data, author) VALUES (?, ?, ?, ?, ?)"
      ).bind(location, date, type, JSON.stringify(data2), author || "System").run();
    }
    return c.json({ success: true });
  } catch (e) {
    return c.json({ error: e.message }, 500);
...
app.post("/api/hall-status", async (c) => {
  try {
    ...
    if (c.env.DB) {
      await c.env.DB.prepare("INSERT OR REPLACE INTO hall_status (hall_id, location, status, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)").bind(hall_id, location || "kp", status).run();
    }
    return c.json({ success: true });
  } catch (e) {
    return c.json({ error: e.message }, 500);
...
app.post("/api/personal-need", async (c) => {
  try {
    ...
    if (c.env.DB) {
      await c.env.DB.prepare("INSERT OR REPLACE INTO personal_need (location, active, message, recipient, updated_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)").bind(location || "kp", active ? 1 : 0, message || "", recipient || "TL").run();
    }
    return c.json({ success: true });
  } catch (e) {
    return c.json({ error: e.message }, 500);
...
app.post("/api/feedback", async (c) => {
  try {
    const body = await c.req.json();
    const { text: text3, contact } = body;
    if (!text3) return c.json({ error: "Text is required" }, 400);
    if (c.env && c.env.DB) {
    ...
    if (key) {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `
                        <p>Du hast ein neues Feedback f\xFCr das Dashboard erhalten:</p>
...
function urlBase64(buffer) {
  return Buffer2.from(buffer).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
}
__name(urlBase64, "urlBase64");
__name2(urlBase64, "urlBase64");
...
__name(createVapidHeader, "createVapidHeader");
__name2(createVapidHeader, "createVapidHeader");
...
async function encryptPayload(sub, payload) {
  ...
  const record = new Uint8Array([...payloadBytes, 2]);
...
app.get("/api/push/last-notification", async (c) => {
  return c.json({
    ...
    body: "Dies ist eine Test-Benachrichtigung mit Bild! \u{1F3AC}",
  ...
  try {
    ...
    for (const sub of subscriptions.results) {
      try {
        const authHeader = await createVapidHeader(sub.endpoint, c.env);
        const payload = {
          title: title || "\u{1F4E2} Kinopolis Nachricht",
          body: message,
          icon: "/logo-kinopolis-official.png",
          data: { url: "/" }
        };
...
app.post("/api/scan-plan", async (c) => {
  try {
    ...
    const prompt = 'Dieser Foto zeigt einen gedruckten Kinopolis \'Auslassplan\'. Extrahiere die Tabelle und gib ausschlie\xDFlich ein valides JSON-Array zur\xFCck. Nutze folgendes Format f\xFCr jedes Objekt: { "hall": "...", "movie": "...", "start_time": "...", "credits_time": "...", "end_time": "..." }. Antworte NUR mit dem JSON-String in einem Code-Block (```json ... ```). KEIN WEITERER TEXT.';
    let response;
    try {
    ...
    try {
      if (c.env.DB && Array.isArray(data2)) {
      ...
      return c.json({ success: true, data: data2 });
    } catch (dbError) {
      console.error("D1 Database Error:", dbError);
      return c.json({ success: true, data: data2, dbWarning: "Konnte Plan nicht in Datenbank speichern." });
    }
  } catch (e) {
...
function decodeKinopolisText(str) {
  if (!str) return "";
  return str.replace(/&#x([0-9a-fA-F]+);/gi, (_, h) => {
    try {
...
__name(decodeKinopolisText, "decodeKinopolisText");
__name2(decodeKinopolisText, "decodeKinopolisText");
app.get("/api/upcoming", async (c) => {
  try {
    ...
    const html3 = await response.text();
    const soonMatch = html3.match(/<section[^>]*id="coming-soon-slider"[^>]*>([\s\S]*?)<\/section>/i);
    const soonHtml = soonMatch ? soonMatch[1] : "";
    const upcoming = [];
    if (soonHtml) {
      const brickRe = /<div[^>]*class="[^"]*grid__brick[^"]*\bmovie\b[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/g;
      let bm;
      while ((bm = brickRe.exec(soonHtml)) !== null) {
        ...
        const poster = imgM[1].startsWith("http") ? imgM[1] : `https://www.kinopolis.de${imgM[1].startsWith("/") ? "" : "/"}${imgM[1]}`;
  ...
  try {
    ...
    try {
      const logs = await c.env.DB.prepare(`
                ...
                LIMIT 50
            `).bind(location).all();
      if (logs.results.length === 0) return c.json({ summary: "Keine Eintr\xE4ge in den letzten 7 Tagen vorhanden." });
      const logText = logs.results.map((l) => `[${l.priority.toUpperCase()}] ${l.author}: ${l.message}`).join("\n");
      const systemPrompt = `Du bist ein hilfreicher Assistent f\xFCr Kinoleiter. 
            Analysiere die folgenden \xDCbergabebuch-Eintr\xE4ge der letzten Tage. 
            Erstelle eine SEHR kompakte Zusammenfassung (max 3-5 Aufz\xE4hlungspunkte). 
  ...
  try {
    ...
    if (type === "seatmap") {
      prompt = `Du bist ein Sitzplan-Analyst. Analysiere diesen Screenshot eines Kinosaal-Buchungsplans.
            REGELN:
            1. Z\xE4hle die belegten Pl\xE4tze (dunkelgrau oder mit Icon/M\xE4nnchen markiert).
            2. Z\xE4hle die freien Pl\xE4tze (farbig markiert: blau, beige oder gr\xFCn).
            3. Berechne die prozentuale Auslastung.
            4. Gib NUR JSON zur\xFCck: {"occupied": X, "available": Y, "occupancy_percent": Z}`;
    } else {
      prompt = `Du bist ein hochpr\xE4ziser OCR-Assistent f\xFCr Kinobetriebe. 
            Analysiere das beigef\xFCgte Foto eines gedruckten Plans.
            AUFGABE: Extrahiere NUR die tats\xE4chlich im Bild sichtbaren Daten f\xFCr Saal, Film und Credits-Zeit.
            REGELN:
            ...
            3. Gib NUR ein valides JSON-Array zur\xFCck.
            Format: [{"hall": "1", "movie": "Titel", "credits_time": "HH:MM"}, ...]
            4. Wenn kein Plan erkennbar ist, gib ein leeres Array [] zur\xFCck.`;
    }
    let result;
    ...
    try {
      ...
      return c.json({ error: `KI-Verarbeitungsfehler: ${e.message}. M\xF6glicherweise ist das Bild zu gro\xDF oder die KI \xFCberlastet.` }, 500);
    }
    const jsonMatch = result.match(/(\[[\s\S]*\]|\{[\s\S]*\})/);
    if (!jsonMatch) {
      return c.json({ error: "KI konnte keine g\xFCltige Datenstruktur finden.", raw: result }, 500);
    }
    const data2 = JSON.parse(jsonMatch[0]);
    ...
    return c.json({ success: true, data: data2, model: usedModel });
  } catch (e) {
    console.error("Scan error:", e);
...
app.post("/api/push/restock", async (c) => {
  ...
  const payload = {
    title: "\u{1F6A8} Nachschub ben\xF6tigt!",
    body: `${item} an der Theke/Kasse leer! Bitte auff\xFCllen.`,
    tag: "restock-alert",
    data: { url: "/#restock" }
  };
...
app.post("/api/push/transfer", async (c) => {
  ...
  const payload = {
    title: `\u{1F4DD} ${titleStr} - von ${author}`,
    body: items,
    tag: "transfer-alert",
    data: { url: "/#transfer" }
  };
...
async function sendPushToAll(env, payload, locationFilter = null) {
  ...
  const results = [];
  const berlinTime = (/* @__PURE__ */ new Date()).toLocaleString("en-GB", { timeZone: "Europe/Berlin", hour: "2-digit", minute: "2-digit" });
...
__name(sendPushToAll, "sendPushToAll");
__name2(sendPushToAll, "sendPushToAll");
var worker_default = {
  ...
  async scheduled(event, env, ctx) {
          ...
          for (let i = 0; i < sessions.length; i++) {
            ...
            if (diff > 0 && diff <= 15 && s.sold > 50) {
              ...
              if (!existing) {
                ...
                await sendPushToAll(env, {
                  ...
                  body: `${s.title} beginnt in ${diff} Min. (${s.sold} G\xE4ste). Bitte Ausweise kontrollieren!`,
                  tag: "ze-alert"
                }, loc);
              }
            }
            ...
            if (endDiff > -5 && endDiff <= 5 && s.duration > 0) {
              ...
              if (!existing) {
                await env.DB.prepare("INSERT INTO notification_state (alert_hash) VALUES (?)").bind(alertHash).run();
                await sendPushToAll(env, {
                  title: "\u{1F6AA} Auslass l\xE4uft: " + hall.name,
    ...
    for (const row of scannedResults.results) {
      ...
      const [h2, m2] = row.credits_time.split(":").map(Number);
      const alertMin = h2 * 60 + m2;
...
var import_checked_fetch122 = __toESM2(require_checked_fetch2());
...
var import_checked_fetch123 = __toESM2(require_checked_fetch2());
...
var import_checked_fetch125 = __toESM2(require_checked_fetch2());
...
//# sourceMappingURL=bundledWorker-0.4903447103393962.js.map
